...
//        c_this.console.log(gridIndex)//todo removed log
    ...
    UpdateProbabilityMap(probabilities, updateMap)
    {
        updateMap.forEach(function(probability, quality, map)
        {
            if(probabilities.get(quality) != undefined)
            {
                ...
                probabilities.set(quality, currentValue + probability);
            }
            else
            {
                probabilities.set(quality, probability);
            }
        })
    ...
    DetermineChordQualities(sample)
    {
          var qualityLut =
          [
              new Map([[ChordTypes.ROOT,3],[ChordTypes.FIRST_INVERSION,2],[ChordTypes.SEVENTH,1],[ChordTypes.FIRST_INVERSION_SEVENTH,0.5]]),     //0
          ...
          sample.A.forEach(function(subSample)
          {
              var bassInterval = undefined == subSample.BassInterval ? 0 : subSample.BassInterval;
              var qualitiesIndex = bassInterval % qualityLut.length;
              var qualitiesToProbabilitiesMap = qualityLut[qualitiesIndex];
              //console.log(qualitiesToProbabilitiesMap, qualitiesIndex)
              this.UpdateProbabilityMap(probabilities, qualitiesToProbabilitiesMap);
          }, this);
    ...
    MakeContextualSelection(sample, lastChordLabel)
    {
        ...
        {
            return
              sample.Probabilities.get(keyB) -
              sample.Probabilities.get(keyA);
        });
    ...
    AnalyzeIntervals(noteArray)
    {
        ...
        while(noteIndex < arrayLength)
        {
            ...
            var selectedChordQuality = this.MakeContextualSelection(sample, lastChordLabel);
